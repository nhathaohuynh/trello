...
export class UserController {
  ...
  async signUp(req: Request, res: Response) {
    ...
    return new CreatedResponse(data, CONSTANT.MSG_CREATE_USER_SUCCESS).send(res)
  }

  async signIn(req: Request, res: Response) {
    const data = await this.userService.signIn(req.body)
    return new OKResponse(data.user).setToken(res, data.accessToken, data.refreshToken).send(res)
  }
  ...
  async logout(req: Request, res: Response) {
    return new OKResponse({ id: req.userId }).clearToken(res).send(res)
  }
